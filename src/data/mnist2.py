...
class ColoredMNIST(Dataset):
    def __init__(self, file_path, data_config, target=False):
        ...
        self.data = self.dataset_generate(file_path=file_path,target=target)
    ...
    def dataset_generate(self, target, file_path):
        if target == False:
            data = self.load_data(file_path=file_path, train=True,
                                    label_to_y=self.label_to_y)
            return data
        else:
            data = self.load_data(file_path=file_path,train=False,label_to_y=self.label_to_y)
            return data
    ...
    def load_data(self, file_path, train, label_to_y):
        ...
        for k, v in data.items():
            random.shuffle(v)
            data_train[k]= torch.stack(v[:0.8*len(v)],dim = 0)
            data_test[k] = torch.stack(v[0.8*len(v):],dim = 0)
            # data[k] = torch.stack(v, dim=0)
        total_data = {'train':data_train,'test':data_test}

        ## data: dict k为key，新定的label， v为打乱后对应label的list
        return total_data
